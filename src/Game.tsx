...
import { BOARD_HEIGHT, BOARD_WIDTH, GAME_SPEED, RIVER_HEIGHT, ROAD_HEIGHT, TILES } from './constants.js';
...
const Game: React.FC = () => {
  const [frogPosition, setFrogPosition] = useState<Position>({ x: BOARD_WIDTH / 2, y: BOARD_HEIGHT - 1 });
  ...
  const checkCollisions = useCallback(() => {
    ...
    if (y > 0 && y <= RIVER_HEIGHT) {
      const logCollision = obstacles.some(
        obstacle => obstacle.type === 'log' && obstacle.position.x === x && obstacle.position.y === y
      );
      if (!logCollision) {
        setGameOver(true);
      }
    ...
    const carCollision = obstacles.some(
      obstacle => obstacle.type === 'car' && obstacle.position.x === x && obstacle.position.y === y
    );
    if (carCollision) {
      setGameOver(true);
    }
  ...
  useInput((input, key) => {
    ...
    if (key.leftArrow) {
      setFrogPosition(prev => ({ ...prev, x: Math.max(0, prev.x - 1) }));
    } else if (key.rightArrow) {
      setFrogPosition(prev => ({ ...prev, x: Math.min(BOARD_WIDTH - 1, prev.x + 1) }));
    } else if (key.upArrow) {
      setFrogPosition(prev => ({ ...prev, y: Math.max(0, prev.y - 1) }));
    } else if (key.downArrow) {
      setFrogPosition(prev => ({ ...prev, y: Math.min(BOARD_HEIGHT - 1, prev.y + 1) }));
    }
  });
