...
export const checkCarCollision = (position: Position, obstacles: Obstacle[]): boolean => {
...
export const checkLogCollision = (position: Position, obstacles: Obstacle[]): Obstacle | undefined => {
...
export const isFrogOnLog = (position: Position, logId: string | undefined, obstacles: Obstacle[]): boolean => {
  if (!logId) return false
  const log = obstacles.find((o) => o.id === logId)
  return !!log && position.x >= log.position.x && position.x < log.position.x + log.length
}
